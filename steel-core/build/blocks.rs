...
pub fn build(blocks: &[BlockClass]) -> String {
    ...
    let mut iron_bars_blocks = Vec::new();
    ...
    for block in blocks {
        ...
        match block.class.as_str() {
            ...
            "IronBarsBlock" => iron_bars_blocks.push(const_ident),
    ...
    let iron_bars_type = Ident::new("IronBarsBlock", Span::call_site());
    ...
    let iron_bars_registrations = generate_registrations(iron_bars_blocks.iter(), &iron_bars_type);
    ...
    let output = quote! {
        ...
        use crate::behavior::blocks::{CraftingTableBlock, FenceBlock, IronBarsBlock, RotatedPillarBlock};

        pub fn register_block_behaviors(registry: &mut BlockBehaviorRegistry) {
            #crafting_table_registrations
            #fence_registrations
            #iron_bars_registrations
