...
use crate::worldgen::noise::simd_aquifer::{SimdAquifer, simd_preliminary_surface_level};
use crate::worldgen::noise::simd_noise_chunk::SimdNoiseChunk;
use crate::worldgen::surface::SurfaceSystem;
use steel_worldgen::simd_overworld::{
    OverworldNoiseSettings, SimdOverworldColumnCache, SimdOverworldNoises,
};
...
pub struct SimdVanillaGenerator {
    ...
    noises: Box<SimdOverworldNoises>,
    ...
    biome_zoom_seed: i64,
    _phantom: PhantomData<SimdOverworldNoises>,
}

impl SimdVanillaGenerator {
    ...
    pub fn new(biome_source: BiomeSourceKind, seed: u64) -> Self {
        ...
        let splitter = if OverworldNoiseSettings::LEGACY_RANDOM_SOURCE {
            LegacyRandom::from_seed(seed).next_positional()
        } else {
            Xoroshiro::from_seed(seed).next_positional()
        };
        let noise_params = get_noise_parameters();
        let noises = SimdOverworldNoises::create(seed, &splitter, &noise_params);

        let ore_veinifier = if OverworldNoiseSettings::ORE_VEINS_ENABLED {
            Some(OreVeinifier::new(&splitter))
        } else {
            None
        };

        let default_block_id = OverworldNoiseSettings::default_block_id();
        let surface_system = SurfaceSystem::new(
            &splitter,
            &noise_params,
            SimdOverworldNoises::surface_noise_ids(),
            default_block_id,
...
impl ChunkGenerator for SimdVanillaGenerator {
    ...
    fn fill_from_noise(&self, chunk: &ChunkAccess) {
        ...
        let mut noise_chunk = SimdNoiseChunk::new(chunk_min_x, chunk_min_z);
        let noises = &*self.noises;

        let mut column_cache = SimdOverworldColumnCache::default();
        ...
        let mut aquifer = SimdAquifer::new(
            chunk_min_x,
            chunk_min_z,
            min_y,
            height,
    ...
    fn build_surface(&self, chunk: &ChunkAccess, neighbor_biomes: &dyn Fn(i32, i32, i32) -> u16) {
        ...
        let mut psl_cache = SimdOverworldColumnCache::default();
        let p00 = simd_preliminary_surface_level(noises, &mut psl_cache, chunk_min_x, chunk_min_z);
        let p10 =
            simd_preliminary_surface_level(noises, &mut psl_cache, chunk_min_x + 16, chunk_min_z);
        let p01 =
            simd_preliminary_surface_level(noises, &mut psl_cache, chunk_min_x, chunk_min_z + 16);
        let p11 = simd_preliminary_surface_level(
            noises,
            &mut psl_cache,
            chunk_min_x + 16,
            chunk_min_z + 16,
