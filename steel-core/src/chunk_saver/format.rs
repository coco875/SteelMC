...
pub enum PersistentProcessorList {
    ...
    Direct(Vec<PersistentStructureProcessorKind>),
    /// Registry-backed processor list.
    Registry(Identifier),
}

/// Persisted direct structure processor payload.
#[derive(SchemaWrite, SchemaRead)]
pub enum PersistentStructureProcessorKind {
    /// Applies the first matching rule.
    Rule {
        /// Ordered processor rules.
        rules: Vec<PersistentProcessorRuleData>,
    },
}

/// Persisted rule inside a rule processor.
#[derive(SchemaWrite, SchemaRead)]
pub struct PersistentProcessorRuleData {
    /// Template block predicate.
    pub input_predicate: PersistentStructureRuleTestData,
    /// World block predicate.
    pub location_predicate: PersistentStructureRuleTestData,
    /// Position predicate.
    pub position_predicate: PersistentPosRuleTestData,
    /// Replacement block state.
    pub output_state: PersistentProcessorBlockStateData,
    /// Output block-entity modifier.
    pub block_entity_modifier: PersistentRuleBlockEntityModifierData,
}

/// Persisted block-state rule test.
#[derive(SchemaWrite, SchemaRead)]
pub enum PersistentStructureRuleTestData {
    /// Always matches.
    AlwaysTrue,
    /// Matches a block.
    BlockMatch {
    ...
    /// Matches a full block state.
    BlockStateMatch {
        /// Block state.
        block_state: PersistentProcessorBlockStateData,
    },
    /// Randomly matches a full block state.
...
/// Persisted position rule test.
#[derive(SchemaWrite, SchemaRead)]
pub enum PersistentPosRuleTestData {
    /// Always matches.
    AlwaysTrue,
    /// Linear chance by axis-aligned distance.
    AxisAlignedLinearPos {
        /// Axis ordinal: 0=x, 1=y, 2=z.
        axis: i8,
        /// Chance at minimum distance.
        min_chance: f32,
        /// Chance at maximum distance.
        max_chance: f32,
        /// Minimum distance.
        min_dist: i32,
        /// Maximum distance.
...
/// Persisted block-entity modifier for rule processors.
#[derive(SchemaWrite, SchemaRead)]
pub enum PersistentRuleBlockEntityModifierData {
    /// Leaves block-entity data unchanged.
    Passthrough,
    /// Appends loot-table metadata.
    AppendLoot {
...
/// Persisted processor block-state data.
#[derive(SchemaWrite, SchemaRead)]
pub struct PersistentProcessorBlockStateData {
    /// Block identifier.
    pub name: Identifier,
