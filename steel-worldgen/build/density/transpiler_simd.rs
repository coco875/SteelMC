...
use crate::density::TranspilerInput;
...
#[must_use]
pub fn transpile_simd(input: &TranspilerInput) -> TokenStream {
    ...
    quote! {
        ...
        use steel_math::simd_utils::{SimpleSimdElement, SimpleSimd};
...
impl TranspileContext {
    ...
    fn gen_column_cache(&mut self, input: &TranspilerInput) -> TokenStream {
        ...
        let grid_fields: Vec<TokenStream> = flat_names
            ...
            .map(|name| {
                ...
                quote! { #field: Simd<f64, #grid_total_lit> }
            })
            .collect();
        ...
        let inline_noise_ensure_stmts: Vec<TokenStream> = self
            ...
            .map(|(idx, noise_id, xz_scale)| {
                ...
                quote! {
                    self.#grid = noises.#noise_field.get_value_simd(
                        x.cast::<f64>() * Simd::splat(#scale), Simd::splat(0.0), z.cast::<f64>() * Simd::splat(#scale),
                    );
                }
            })
            .collect();
        ...
        quote! {
            ...
            impl #cache {
                ...
                pub fn init_grid(&mut self, chunk_block_x: i32, chunk_block_z: i32,
                    ...
                    let total_elements = (Self::GRID_SIDE * Self::GRID_SIDE) as usize;

                    let mut idx_arr = Vec::with_capacity(total_elements);
                    let mut x_arr = Vec::with_capacity(total_elements);
                    let mut z_arr = Vec::with_capacity(total_elements);

                    for rel_z in 0..Self::GRID_SIDE {
                        for rel_x in 0..Self::GRID_SIDE {
                            let x = ((self.grid_first_quart_x + rel_x) << 2) as i32;
                            let z = ((self.grid_first_quart_z + rel_z) << 2) as i32;
                    ...
                    let x = Simd::from_array(x_arr.try_into().unwrap());
                    let z = Simd::from_array(z_arr.try_into().unwrap());
    ...
    fn fn_params(&self, is_flat: bool) -> TokenStream {
        ...
        let cache = &self.cache_ident;
        if is_flat {
            quote! { noises: &#noises, cache: &#cache, x: Simd<i32, N>, z: Simd<i32, N> }
        } else {
            quote! { noises: &#noises, cache: &#cache, x: Simd<i32, N>, y: Simd<i32, N>, z: Simd<i32, N> }
        }
    }
    ...
    fn gen_named_functions(&mut self, input: &TranspilerInput) -> TokenStream {
        ...
        for name in self.topo_order.clone() {
            ...
            fns.push(quote! {
                #[doc = #doc]
                #[inline]
                fn #fn_name<F, const N: usize>(#params) -> Simd<F, N> where F: SimpleSimdElement, Simd<F, N>: SimpleSimd<F, N> {
                    #body
                }
            });
        }
    ...
    fn gen_router_functions(&mut self, input: &TranspilerInput) -> TokenStream {
        ...
        for (name, df) in &input.router_entries {
            ...
            if self.flat_routers.contains(name) {
                ...
                fns.push(quote! {
                    #[inline]
                    fn #compute_fn_name<F, const N: usize>(#compute_params) -> Simd<F, N> where F: SimpleSimdElement, Simd<F, N>: SimpleSimd<F, N> {
                        #compute_body
                    }
                });
                ...
                fns.push(quote! {
                    #[doc = #doc]
                    #[inline]
                    pub fn #fn_name<F, const N: usize>(#full_params) -> Simd<F, N> where F: SimpleSimdElement, Simd<F, N>: SimpleSimd<F, N> {
                        cache.#cache_field
                    }
                });
            } else {
    ...
    fn gen_expr(
        ...
        match df {
            ...
            DensityFunction::YClampedGradient(g) => {
                ...
                quote! { map_clamped_simd(y.cast(), #from_y, #to_y, #from_val, #to_val) }
            }
            ...
            DensityFunction::Noise(n) => {
                ...
                if is_flat || n.y_scale == 0.0 {
                    quote! { noises.#field.get_value_simd(x.cast() * Simd::splat(#xz_scale).cast(), Simd::splat(0.0).cast(), z.cast() * Simd::splat(#xz_scale).cast()) }
                } else {
                    quote! { noises.#field.get_value_simd(x.cast() * Simd::splat(#xz_scale).cast(), y.cast() * Simd::splat(#y_scale).cast(), z.cast() * Simd::splat(#xz_scale).cast()) }
                }
            }
                ...
                if is_flat || sn.y_scale == 0.0 {
                    quote! {{
                        ...
                        noises.#field.get_value_simd(
                            x.cast() * Simd::splat(#xz_scale) + Simd::splat(dx),
                            Simd::splat(0.0),
                            z.cast() * Simd::splat(#xz_scale) + Simd::splat(dz),
                        )
                    }}
                } else {
                    quote! {{
                        ...
                        noises.#field.get_value_simd(
                            x.cast() * Simd::splat(#xz_scale) + Simd::splat(dx),
                            y.cast() * Simd::splat(#y_scale) + Simd::splat(dy),
                            z.cast() * Simd::splat(#xz_scale) + Simd::splat(dz),
                        )
                    }}
                }
            ...
            DensityFunction::ShiftA(s) => {
                ...
                quote! { noises.#field.get_value_simd(x.cast() * Simd::splat(0.25).cast(), Simd::splat(0.0).cast(), z.cast() * Simd::splat(0.25).cast()) * Simd::splat(4.0).cast() }
            }

            DensityFunction::ShiftB(s) => {
                let field = noise_field_ident(&s.noise_id);
                quote! { noises.#field.get_value_simd(z.cast() * Simd::splat(0.25).cast(), x.cast() * Simd::splat(0.25).cast(), Simd::splat(0.0).cast()) * Simd::splat(4.0).cast() }
            }
            ...
            DensityFunction::TwoArgumentSimple(t) => {
                ...
                if hoisted.is_empty() {
                    match t.op {
                        ...
                        TwoArgType::Min => quote! { Simd::min(#a, #b) },
                        TwoArgType::Max => quote! { Simd::max(#a, #b) },
                    }
                } else {
                    let op = match t.op {
                        TwoArgType::Add => quote! { ((#a) + (#b)) },
            ...
            DensityFunction::WeirdScaledSampler(ws) => {
                ...
                quote! {{
                    ...
                    let scale = #mapper.get_values_simd(rarity);
                    scale * noises.#field.get_value_simd(
                        x.cast() / scale, y.cast() / scale, z.cast() / scale,
                    ).abs()
                }}
            }
