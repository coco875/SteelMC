...
fn identifier_token(s: &str) -> TokenStream {
    let id = Identifier::parse_or_vanilla(s)
        .unwrap_or_else(|error| panic!("invalid identifier {s:?}: {error}"));
...
fn jukebox_song_ref_token(value: &Value) -> TokenStream {
    ...
    let id = Identifier::parse_or_vanilla(song)
...
fn instrument_ref_token(value: &Value) -> TokenStream {
    ...
    let id = Identifier::parse_or_vanilla(instrument)
...
fn trim_material_ref_token(value: &Value) -> TokenStream {
    ...
    let id = Identifier::parse_or_vanilla(material)
...
fn registry_sound_event_holder_token(sound: &str, field: &str) -> TokenStream {
    let id = Identifier::parse_or_vanilla(sound).unwrap_or_else(|error| {
...
fn sound_event_value_token(value: &Value, field: &str) -> TokenStream {
    ...
    Identifier::parse_or_vanilla(sound_id_value).unwrap_or_else(|error| {
...
fn generate_tool_rule(rule: &Value) -> TokenStream {
    let blocks_token = match rule.get("blocks") {
        Some(Value::String(value)) if value.starts_with('#') => {
            ...
            Identifier::parse_or_vanilla(tag_value)
    ...
    for (key, value) in &item.components {
        ...
        match key.as_str() {
            ...
            "minecraft:item_model" => {
                ...
                let model = Identifier::parse_or_vanilla(model)
            ...
            "minecraft:chicken/variant" => {
                ...
                let variant = Identifier::parse_or_vanilla(variant)
            ...
            "minecraft:use_cooldown" => {
                ...
                let cooldown_group = value
                    ...
                    .map_or_else(
                        || quote! { None },
                        |group| {
                            let id = Identifier::parse_or_vanilla(group)
